(ns construction-supervision.actor
  "ConstructionSupervisionActor — the ISCO-08 3123 independent
  construction-supervision actor as a `langgraph.graph/state-graph`
  ...
  The unconditional invariant: the SiteSupervisionAdvisor can never
  directly commit a record or dispatch a robot action the
  ConstructionSupervisionGovernor refuses — every commit-record! call
  is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [construction-supervision.advisor :as advisor]
            [construction-supervision.governor :as governor]
            [construction-supervision.store :as store]))

(defn build-graph
  "Build a compiled ConstructionSupervisionActor graph. `store`
  implements `construction-supervision.store/Store`. `advisor`
  implements `construction-supervision.advisor/Advisor` (defaults to
